<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\OrderGoods;

/**
 * OrderGoodsSearch represents the model behind the search form of `app\models\OrderGoods`.
 */
class OrderGoodsSearch extends OrderGoods
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'goodsId', 'is_over', 'drawnum', 'surplusnum'], 'integer'],
            [['orderId', 'goodstotal', 'money'], 'safe'],
            [['surplusprice', 'drawprice', 'price'], 'number'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = OrderGoods::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'goodsId' => $this->goodsId,
            'is_over' => $this->is_over,
            'surplusprice' => $this->surplusprice,
            'drawprice' => $this->drawprice,
            'price' => $this->price,
            'drawnum' => $this->drawnum,
            'surplusnum' => $this->surplusnum,
        ]);

        $query->andFilterWhere(['like', 'orderId', $this->orderId])
            ->andFilterWhere(['like', 'goodstotal', $this->goodstotal])
            ->andFilterWhere(['like', 'money', $this->money]);

        return $dataProvider;
    }
}
